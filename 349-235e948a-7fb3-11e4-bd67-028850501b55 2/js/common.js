...
$(function(){
	$("#course_id").on('change',function(){
		...
		{
			$.ajax({
				url:	BASE_URL+"home/getStreamByCourseId",
				type:	'POST',
				data:	{course_id:course_id},
				success:	function(resp)
				{
					var str	='<option value="0">Select Stream</option>';
	...
	$(".auth").on('change','#stream_id',function(){ //.row-fluid ,
		var stream_id	= $(this).val();
		if(stream_id > 0)
		{
			$.ajax({
				url:	BASE_URL+"home/getProgramByStreamId",
				type:	'POST',
				data:	{stream_id:stream_id},
				success:	function(resp)
				{
					var str	='<option value="0">Select Program Level</option>';
					var obj	= $.parseJSON( resp );
					$.each(obj,function(index,value){
						str +='<option value="'+value.id+'">'+value.level+'</option>';
					})
					$("#program_id").html(str);
				},
				error:		function()
	...
	$("#enquire_course").on('click',function(e){
		e.preventDefault();
		...
		if(course_id)
		{
			...
			if(stream)
			{
				...
				//if(program)
				//{
					//window.location.href=BASE_URL+"enquire/find_your_course/course/"+course_id+"/stream/"+stream+"/program/"+program;
					window.location.href=BASE_URL+"course/all_streams/"+course_id+"/show_stream/"+stream;
				//}
				//else
				//{
					//alert("Select program level");
				//}
			}
			else
			{
